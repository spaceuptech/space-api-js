...
class Api {
  ...
  constructor(projectId, url = "/") {
    ...
    this.fileStore = new FileStore(projectId, url, this.options);
  }
  ...
  call(service, func, params, timeout) {
    ...
    const url = `${this.url}v1/api/functions/${service}/${func}`;
  ...
  Service(service) {
    return new Service(this.appId, this.client, service)
  }

  /**
   * Returns a FileStore client instance
   * @returns {external:FileStore} SQL client instance
   */
  FileStore() {
