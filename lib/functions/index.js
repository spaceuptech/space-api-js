class Service {
  constructor(options, client, service) {
    ...
    this.client.registerOnReconnectCallback(() => {
      this.client.request('service-register', { service: this.service, project: this.options.project, token: this.options.token }).then(data => {
        if (!data.ack) {
  ...
  start() {
    ...
    this.client.request('service-register',
      { service: this.service, project: this.options.project, token: this.options.token }).then(data => {
        if (!data.ack) {
          console.log('Could not connect to service')
          return
        }
