  ...
  if (!opts.changesOnly) {
    ...
    if (changeType === 'initial') {
      ...
      if (changeType !== 'delete') {
        obj.subscription.onSnapshot(obj.snapshot.filter(row => (!row.isDeleted)).map(row => row.payload), changeType, rows[0].payload)
      } else {
        if (rows[0].dbType === 'mongo') {
          obj.subscription.onSnapshot(obj.snapshot.filter(row => (!row.isDeleted)).map(row => row.payload), changeType, { _id: rows[0].docId })
        } else {
          obj.subscription.onSnapshot(obj.snapshot.filter(row => (!row.isDeleted)).map(row => row.payload), changeType, { id: Number(rows[0].docId) })
        }
      }
    }
  }
